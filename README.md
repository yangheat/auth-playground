...
HTTP/HTTPS에 따라 요청에 쿠키 자동 포함 여부를 설정  
- HTTPS: https://auth-playground.test:5173
- HTTP: http://auth-playground.test:5175

- 로그인 후 GET /session 을 통해 로그인 상태 확인
  | 조건 | 결과 |
  | --- | --- |
  | Secure=false | HTTPS와 HTTP 모두 Request Headers > Cookie에 sessionId를 자동 포함하여 성공 |
  | Secure=true | HTTPS에만 Request Headers > Cookie에 sessionId를 자동 포함하여 성공. HTTP는 포함하지 않아 실패.  |
...
- None은 Secure=true와 HTTPS가 필요하며, cross-site fetch는 credentials: "include"와 CORS 허용 시 쿠키가 포함
  
- 자동 포함 여부
  | 테스트 | 요청 방식 | Strict | Lax(기본값) | None | 확인 내용 |
  | --- | --- | --- | --- | --- | --- |
  | Top-level GET /session 이동 | window.open | 안 붙음 | 붙음 | 붙음 | Request Headers > Cookie |
